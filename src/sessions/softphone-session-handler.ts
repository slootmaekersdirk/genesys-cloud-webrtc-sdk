...
export default class SoftphoneSessionHandler extends BaseSessionHandler {
  ...
  debouncedEmitCallError: DebouncedFunc<
    (
  ...
  constructor(sdk: GenesysCloudWebrtcSdk, sessionManager: SessionManager) {
    ...
    this.debouncedEmitCallError = debounce(this.emitCallError, 500, {
      leading: true,
    });
  }
  ...
  handleConversationUpdate(
    ...
    if (!participant) {
      return this.log(
        "debug",
        "user participant not found on the conversation update",
        update,
        { skipServer: true }
      );
    }
    ...
    if (!callState) {
      return this.log(
        "debug",
        "user participant's call state not found on the conversation update. not processing",
        { update, participant },
        { skipServer: true }
      );
    }
      ...
      */
      if (
        ...
        !Object.values(this.conversations).find(
          (c) => c.session === this.activeSession
        )
      ) {
        session = this.activeSession;
        this.log(
    ...
    this.handleSoftphoneConversationUpdate(
      update,
      participant,
      callState,
      session
  ...
  handleSoftphoneConversationUpdate(
    ...
    this.log("debug", "about to process conversation event", {
      sessionId: session?.id,
      update,
    ...
    const conversationState: IStoredConversationState = {
      ...
      session:
        this.conversations[conversationId]?.session ||
        lastConversationUpdate?.session ||
        session,
    };

    const previousCallState = this.getUsersCallStateFromConversationEvent(
      lastConversationUpdate?.conversationUpdate
    );
    ...
    const communicationStateChanged =
      previousCallState?.state !== callState.state;
        ...
        */
        if (
          ...
          if (!session) {
            this.log(
              "info",
              "incoming conversation started, but we do not have a session. assuming it was handled by a different client. ignoring",
              {
          ...
          if (session === this.activeSession) {
            ...
            this.sessionManager.onHandledPendingSession(
              session.id,
              conversationId
            );
          }
        ...
        if (this.isPendingState(previousCallState)) {
          if (session && session === this.activeSession) {
            this.sessionManager.onCancelPendingSession(
              session.id,
              conversationId
            );
          }
          ...
          */
          if (session && session === this.activeSession) {
            session.conversationId = conversationId;
            this.sdk.emit("sessionEnded", session, {
    ...
    if (eventToEmit) {
      this.log(
        "debug",
        "about to emit based on conversation event",
        {
          eventToEmit,
          update: this.conversations[conversationId],
          previousCallState,
          callState,
          session,
        },
  ...
  diffConversationCallStates(
    call1: ICallStateFromParticipant,
    call2: ICallStateFromParticipant
  ): boolean {
    return (
      !call1 ||
      !call2 ||
  ...
  emitConversationEvent(
    event: SdkConversationEvents,
    conversation: IStoredConversationState,
    session: IExtendedMediaSession
  ): void {
    ...
    if (event === "added") {
      ...
      currentEmittedEvent.current = currentEmittedEvent.current.filter(
        (conversationEvent) =>
          conversationEvent.conversationId !== conversation.conversationId
      );
      delete this.conversations[conversation.conversationId];
    }

    currentEmittedEvent.activeConversationId =
      this.determineActiveConversationId(session);

    this.log(
      "debug",
      "emitting `conversationUpdate`",
      {
        event,
        previousEmittedEvent: this.lastEmittedSdkConversationEvent,
        currentEmittedEvent,
        session,
      },
      { skipServer: true }
  ...
  determineActiveConversationId(session?: IExtendedMediaSession): string {
    ...
    if (conversations.length === 0) {
      return (
        this.activeSession?.conversationId || session?.conversationId || ""
      );
    ...
    const connectedConversations = conversations.filter((c) =>
      this.isConnectedState(c.mostRecentCallState)
    );
    ...
    if (connectedConversations.length === 1) {
      ...
      const nonHeldConversations = connectedConversations.find(
        (c) => c.mostRecentCallState.held === false
      );
  ...
  getUsersCallStateFromConversationEvent(
    update: ConversationUpdate,
    state?: CommunicationStates
  ): ICallStateFromParticipant | undefined {
  ...
  getUserParticipantFromConversationEvent(
    update: ConversationUpdate,
    state?: CommunicationStates
  ): IConversationParticipantFromEvent | undefined {
    if (!update) {
      return;
    }

    const participantsForUser = update.participants
      .filter((p) => p.userId === this.sdk._personDetails.id)
      .reverse();
    let participant: IConversationParticipantFromEvent;

    if (!participantsForUser.length) {
      this.log("warn", "user not found on conversation as a participant", {
        conversationId: update.id,
      });
      return;
    }
    ...
    if (!participant && state) {
      participant = participantsForUser.filter((p) =>
        p.calls.find((c) => c.state === state)
      )[0];
    }
  ...
  getCallStateFromParticipant(
    ...
    if (!callLen) {
      this.log("debug", "no call found on participant", {
  ...
  private setCurrentSession(session: IExtendedMediaSession): void {
    const loggy = () => ({
      ...
      isConcurrentSoftphoneSessionsEnabled:
        this.sdk.isConcurrentSoftphoneSessionsEnabled(),
      isPersistentConnectionEnabled: this.sdk.isPersistentConnectionEnabled(),
      otherActiveSession: this.sdk.sessionManager
    ...
    session.once("terminated", () => {
      ...
      const otherActiveSessions =
        this.sdk.sessionManager.getAllActiveSessions();
      if (
        this.sdk.isPersistentConnectionEnabled() &&
        !!otherActiveSessions.length
      ) {
        this.log(
          "debug",
          "active session ended. Using next available session as persistent connection",
          {
            terminatedSession: {
              sessionId: session.id,
              conversationId: session.conversationId,
            ...
            selectedSession: {
              sessionId: otherActiveSessions[0].id,
              conversationId: otherActiveSessions[0].conversationId,
            },
          }
        );
  ...
  async acceptSession(
    ...
    if (!stream) {
      this.log("info", "No mediaStream provided, starting media", {
        conversationId: session.conversationId,
        sessionId: session.id,
        sessionType: session.sessionType,
      });
      stream = await this.sdk.media.startMedia({
        audio: this.sdk.media.getValidSdkMediaRequestDeviceId(
          params.audioDeviceId
        ),
        session,
      });
      this.log("info", "Media started", {
        conversationId: session.conversationId,
        sessionId: session.id,
        sessionType: session.sessionType,
      });
  ...
  */
  ...
  async proceedWithSession(pendingSession: IPendingSession) {
    if (
      !this.hasActiveSession() ||
      pendingSession.id !== this.activeSession.id
    ) {
    ...
    this.log(
      "info",
      "`proceedWithSession` called with an active session and LA == 1. sending proceed via HTTP request",
      {
    ...
    if (!participant) {
      participant = await this.fetchUserParticipantFromConversationId(
  ...
  _rejectUcCall(conversationId: string, participantId: string): Promise<any> {
    return requestApi.call(
      this.sdk,
      `/conversations/calls/${conversationId}/participants/${participantId}/replace`,
      {
        method: "post",
  ...
  async endSession(
    ...
    try {
      const participant = await this.getUserParticipantFromConversationId(
        conversationId
      );
      ...
      const terminatedPromise = new Promise<JingleReason>((resolve) => {
        const listener = (
          endedSession: IExtendedMediaSession,
          reason: JingleReason
        ) => {
          if (
            endedSession.id === session.id &&
            endedSession.conversationId === conversationId
          ) {
            this.log(
              "debug",
              'received "sessionEnded" event from session requested by `sdk.endSession()`',
              {
                endedSession,
                conversationId,
                session,
            ...
            this.log(
              "debug",
              'received "sessionEnded" event from session that was NOT requested by `sdk.endSession()`',
              {
                endedSession,
                conversationId,
                session,
      ...
      this.log("error", "Failed to end session gracefully", {
        conversationId,
        sessionId: session.id,
        error,
      });
      ...
      const otherActiveSessions = Object.values(this.conversations)
        .filter((convo) => {
          return (
            ...
            (this.isPendingState(convo.mostRecentCallState) ||
              this.isConnectedState(convo.mostRecentCallState))
          );
        })
        .map((convo) => ({
  ...
  async endSessionFallback(
    conversationId: string,
    session: IExtendedMediaSession,
    reason?: Constants.JingleReasonCondition
  ): Promise<void> {
    this.log("info", "Attempting to end session directly", {
      sessionId: session.id,
      conversationId: session.conversationId,
      sessionType: session.sessionType,
      reason,
    ...
    try {
      ...
      throw createAndEmitSdkError.call(
        this.sdk,
        SdkErrorTypes.session,
        "Failed to end session directly",
        {
          conversationId: session.conversationId,
          sessionId: session.id,
          sessionType: session.sessionType,
  ...
  async fetchUserParticipantFromConversationId(
    conversationId: string
  ): Promise<IConversationParticipantFromEvent> {
    const body = await this.fetchConversationStateFromApi(conversationId);

    return this.getUserParticipantFromConversationEvent(
      body,
      CommunicationStates.connected
    );
  }

  private async getUserParticipantFromConversationId(
    conversationId: string
  ): Promise<IConversationParticipantFromEvent> {
    let userParticipant =
      this.conversations[conversationId]?.mostRecentUserParticipant;

    if (!userParticipant) {
      userParticipant = await this.fetchUserParticipantFromConversationId(
        conversationId
      );
    }
    ...
    if (!userParticipant) {
      throw createAndEmitSdkError.call(
        ...
        "participant not found for converstionId",
        { conversationId }
      );
    }
  ...
  async setAudioMute(
    ...
    try {
      const userParticipant = await this.getUserParticipantFromConversationId(
        conversationId
      );

      return await this.patchPhoneCall(conversationId, userParticipant.id, {
        muted: params.mute,
      });
    } catch (error) {
      throw createAndEmitSdkError.call(
  ...
  async setConversationHeld(
    session: IExtendedMediaSession,
    params: IConversationHeldRequest
  ) {
  ...
  async updateOutgoingMedia(
    session: IExtendedMediaSession,
    options: IUpdateOutgoingMedia
  ): Promise<any> {
    const newOptions: IUpdateOutgoingMedia = {
      ...options,
  ...
  async startSession(
    params: IStartSoftphoneSessionParams
  ): Promise<{ id: string; selfUri: string }> {
    this.log("info", "Creating softphone call from SDK", {
