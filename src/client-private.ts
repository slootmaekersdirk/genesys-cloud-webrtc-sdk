 ...
 */
export async function setupStreamingClient(
  this: GenesysCloudWebrtcSdk
): Promise<void> {
  if (this.isInitialized && this.connected) {
    this.logger.warn(
      "Existing streaming connection detected. Disconnecting and creating a new connection."
    );
  ...
  this.logger.debug(
    "Streaming client WebSocket connection options",
    connectionOptions
  );
  ...
  const initialPromise = new Promise<void>((resolve) => {
    connection.on("connected", async () => {
      ...
      this.logger.info("GenesysCloud streaming client connected", {
        reconnect: this._hasConnected,
      });
 ...
 * @param this must be called with a GenesysCloudWebrtcSdk as `this`
 */
export async function proxyStreamingClientEvents(
  this: GenesysCloudWebrtcSdk
): Promise<void> {
  this.sessionManager = new SessionManager(this);

  if (this._personDetails && !this.isJwtAuth) {
    await this._streamingConnection.notifications.subscribe(
      `v2.users.${this._personDetails.id}.conversations`,
      handleConversationUpdate.bind(this),
      true
    );
  }

  // webrtc events
  const on = this._streamingConnection.webrtcSessions.on.bind(
    this._streamingConnection
  );
  on(
    "requestIncomingRtcSession",
    this.sessionManager.onPropose.bind(this.sessionManager)
  );
  on(
    "incomingRtcSession",
    this.sessionManager.onSessionInit.bind(this.sessionManager)
  );
  ...
  on(
    ...
    this.sessionManager.onCancelPendingSession.bind(this.sessionManager)
  );
  on(
    "handledIncomingRtcSession",
    this.sessionManager.onHandledPendingSession.bind(this.sessionManager)
  );
...
export const handleConversationUpdate = function (
  this: GenesysCloudWebrtcSdk,
  updateEvent: SubscriptionEvent
) {
